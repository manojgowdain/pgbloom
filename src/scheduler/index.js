...
import { createSchedule, getDueJobs, claimJob, completeJob, failJob, cancelJob, getSchedule, listSchedules, } from "./queries.js";
/**
 * Creates a new scheduler state with the database connection pool
 * and optional local store for caching.
 */
export function createSchedulerState(pool, localStore = null, workerId) {
    return {
        pool,
        localStore,
 ...
 */
export async function schedule(state, options) {
 ...
 */
export async function scheduleRecurring(state, options) {
    if (!options.interval) {
 ...
 */
export async function cancelSchedule(state, jobId) {
    await cancelJob(state.pool, jobId);
}
/**
 * Gets job details by ID.
 */
export async function getScheduleJob(state, jobId) {
 ...
 */
export async function listScheduledJobs(state, filter = {}) {
 ...
 */
export async function getAndClaimDueJobs(state, limit = 10) {
 ...
 */
export async function claimScheduledJob(state, jobId) {
 ...
 */
export async function completeScheduledJob(state, jobId) {
    ...
    if (isRecurring && job.interval) {
        ...
        await state.pool.query(`UPDATE pgbloom_schedules
       SET status = 'scheduled',
           last_run_at = NOW(),
           run_at = $1,
           updated_at = NOW()
       WHERE id = $2`, [nextRunAt, jobId]);
 ...
 */
export async function failScheduledJob(state, jobId, error) {
