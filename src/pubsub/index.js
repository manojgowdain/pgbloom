 ...
 */
export function createPubSubState(pool) {
    return {
        pool,
        listeners: new Map(),
        listenClient: null,
 ...
 */
export async function subscribe(state, channel, handler) {
 ...
 */
export async function publish(pool, channel, payload) {
 ...
 */
export async function closePubSub(state) {
    if (state.listenClient) {
