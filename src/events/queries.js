 ...
 */
export async function insertEvent(pool, eventId, type, payload, metadata = {}) {
    const serializedPayload = serialize(payload);
    const result = await pool.query(`INSERT INTO pgbloom_events (event_id, type, payload, metadata)
     VALUES ($1, $2, $3, $4)
     RETURNING id, created_at`, [eventId, type, serializedPayload, JSON.stringify(metadata)]);
 ...
 */
export async function getEventHistory(pool, options = {}) {
 ...
 */
export async function getEventById(pool, eventId) {
    const result = await pool.query(`SELECT id, event_id, type, payload, created_at, metadata
     FROM pgbloom_events
