 ...
 *   k = (m / n) * ln(2)
 */
function calculateParams(expectedItems, falsePositiveRate) {
    if (expectedItems <= 0) {
        throw new BloomFilterConfigError("expectedItems must be a positive integer.");
    }
    if (!(falsePositiveRate > 0 && falsePositiveRate < 1)) {
        throw new BloomFilterConfigError("falsePositiveRate must be a number between 0 and 1 (exclusive).");
    }
    // m = -(n * ln(p)) / (ln(2)^2)
    const bitCount = Math.ceil((-(expectedItems * Math.log(falsePositiveRate))) / LN2_SQUARED);
 ...
 */
function roundUpToByte(bitCount) {
...
export class BloomFilter {
    ...
    constructor(options = {}) {
        ...
        const { bitCount, hashCount } = calculateParams(expectedItems, falsePositiveRate);
    ...
    getIndices(value) {
        ...
        const indices = new Array(this.hashCount);
    ...
    static fromJSON(json) {
        if (!json || json.version !== 1) {
            throw new BloomFilterConfigError("Invalid or unsupported Bloom Filter JSON version.");
        }
        if (typeof json.expectedItems !== "number" ||
            typeof json.falsePositiveRate !== "number" ||
            typeof json.bitCount !== "number" ||
            typeof json.hashCount !== "number" ||
            typeof json.bits !== "string") {
            throw new BloomFilterConfigError("Bloom Filter JSON missing required fields.");
        }
        ...
        if (filter.bitCount !== json.bitCount) {
            throw new BloomFilterConfigError("Bloom Filter bitCount mismatch in serialized data.");
        }
        if (filter.hashCount !== json.hashCount) {
            throw new BloomFilterConfigError("Bloom Filter hashCount mismatch in serialized data.");
        }
        // Decode base64 back into Uint16Array
        const binary = atob(json.bits);
        if (binary.length !== filter.counters.length * 2) {
            throw new BloomFilterConfigError("Bloom Filter bits length mismatch in serialized data.");
        }
        const view = new DataView(new Uint8Array(binary.length).map((_, i) => binary.charCodeAt(i)).buffer);
