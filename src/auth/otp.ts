 ...
 */
export function createOTPState(
  pool: Pool,
  options: OTPOptions = {}
): Required<OTPOptions> {
  return {
...
export async function sendOTP(
  ...
  const existing = await state.pool.query(
    `SELECT id, created_at FROM pgbloom_otp_codes
  ...
  const result = await state.pool.query(
    `INSERT INTO pgbloom_otp_codes (user_id, email, otp_hash, purpose, expires_at, max_attempts)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, expires_at`,
    [userId || null, email, otpHash, purpose, expiresAt, opts.maxAttempts]
  );
 ...
 */
function mapRowToOTPRecord(row: Record<string, any>): OTPRecord {
  return {
    id: row.id,
    userId: row.user_id,
    email: row.email,
...
export async function verifyOTP(
  ...
  purpose: OTTPurpose,
  otpOptions: OTPOptions = {}
): Promise<{ valid: boolean; userId?: string | number; otpRecord?: OTPRecord }> {
  const opts = createOTPState(state.pool, otpOptions);

  // Find the most recent unused OTP for this email/purpose
  const result = await state.pool.query(
    `SELECT * FROM pgbloom_otp_codes
  ...
  await state.pool.query(
    `UPDATE pgbloom_otp_codes SET attempts = attempts + 1 WHERE id = $1`,
    [record.id]
  );
  ...
  // Mark as used
  await state.pool.query(
    `UPDATE pgbloom_otp_codes SET used = TRUE WHERE id = $1`,
    [record.id]
  );
...
export async function resendOTP(
  ...
  await state.pool.query(
    `UPDATE pgbloom_otp_codes SET used = TRUE WHERE email = $1 AND purpose = $2 AND used = FALSE`,
    [email, purpose]
  );
 ...
 */
export async function cleanupExpiredOTPs(state: AuthState): Promise<number> {
  const result = await state.pool.query(
    `DELETE FROM pgbloom_otp_codes WHERE expires_at < NOW() OR used = TRUE`
  );
 ...
 */
export async function getOTPById(state: AuthState, id: string | number): Promise<OTPRecord | null> {
  const result = await state.pool.query(
    `SELECT * FROM pgbloom_otp_codes WHERE id = $1`,
    [id]
  );
