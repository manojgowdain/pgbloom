...
export function createAuthState(
  ...
  if (!jwtSecret) {
    throw new PGBloomAuthError(
      "JWT secret is required. Set auth.jwtSecret in options or JWT_SECRET/PGBLOOM_JWT_SECRET environment variable."
    );
  }
  ...
  const userSchema = {
    ...
    password_hash: { type: "string" as const, required: true },
    name: { type: "string" as const },
    email_verified: { type: "boolean" as const, required: true, default: false },
  };

  const userModel = createModel<UserType>(pool, "pgbloom_users", userSchema, {
...
export class Auth {
  ...
  async signup(input: SignupInput): Promise<AuthResult> {
    ...
    const accessToken = createAccessToken(
      ...
      { jwtSecret: this.state.jwtSecret, accessTokenExpiry: this.state.accessTokenExpiry }
    );
    const refreshToken = createRefreshToken(
      { userId: user.id, email: user.email },
      { jwtSecret: this.state.jwtSecret, refreshTokenExpiry: this.state.refreshTokenExpiry }
    );
  ...
  async login(input: LoginInput): Promise<AuthResult> {
    ...
    const result = await this.state.pool.query(
      `SELECT password_hash FROM pgbloom_users WHERE id = $1`,
      [user.id]
    );
   ...
   */
  async verifyEmail(input: VerifyOTPInput): Promise<void> {
   ...
   */
  async sendLoginOTP(email: string): Promise<{ otpId: string; expiresAt: Date }> {
    const user = await findUserByEmail(this.state.pool, email);
    if (!user) {
      throw new PGBloomAuthError("User not found");
    }
   ...
   */
  async verifyLoginOTP(input: VerifyOTPInput): Promise<AuthResult> {
    const { email, otp } = input;
   ...
   */
  async cleanup(): Promise<{ sessions: number; otps: number }> {
